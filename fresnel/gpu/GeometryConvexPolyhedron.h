...
/*! Define a convex polyhedron geometry.
    ...
    Store a convex polyhedron as a list of planes and perform the needed ray-plane intersection
   tests to find intersections. Determine edge distance by computing plane-plane intersections to
   find edges.
*/
class GeometryConvexPolyhedron : public Geometry
    ...
    protected:
    ManagedBuffer<vec3<float>>
        m_plane_origin; //!< Origins of all the planes in the convex polyhedron
    ManagedBuffer<vec3<float>>
        m_plane_normal; //!< Normals of all the planes in the convex polyhedron
    ManagedBuffer<RGB<float>> m_plane_color; //!< Colors assigned to the polyhedron planes
